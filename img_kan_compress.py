## https://github.com/vladEDT/Kolmogorov.compress
...
class FourierFeatures(nn.Module):
    def __init__(self, in_features=1, mapping_size=64, scale=10.0):
        ...
        # B is fixed (not trained) random projection matrix scaled by `scale`
...
class PerceptualLoss(nn.Module):
    def __init__(self, device):
        ...
        # Use first several layers of VGG16 for perceptual features
    ...
    def forward(self, input, target):
        # input/target: (B, 1, H, W) single-channel; expand to 3 channels and normalize
...
class KolmogorovImageCompressor(nn.Module):
    ...
    def forward(self, coords):
        # coords: (N, 2) where columns are x and y
        x = coords[:, 0:1]
        y = coords[:, 1:2]
...
def compress_image(image_path,
    ...
    """
    Train the generator on a single image using a composite loss.
    If the model file already exists, it will be loaded instead of training from scratch.
    """
...
def fine_tune_image(image_path,
    ...
    print("Fine-tuning finished. Saving updated model...")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(image_path):
        print(f"File {image_path} not found. Place the image in the working folder and provide the correct path.")
        exit(1)
    ...
    fine_tune = input("Perform fine-tuning? (y/n): ").strip().lower()
    if fine_tune == 'y':
